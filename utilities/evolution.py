...
def sort_and_rank(score, target):
    target1 = torch.gather(input=score, dim=1, index=target.view(-1, 1))
    indices1 = ((score > target1).sum(dim=1))
    indices2 = ((score == target1).sum(dim=1))
    indices = (indices1 + (indices2 - 1) / 2 + 1)
    return indices
